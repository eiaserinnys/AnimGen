 ...
 * decimal digits of precision for ExpMapTypes, the Taylor approximation
 ...
 * -----------------------------------------------------------------*/
void Q_To_Matrix(Quat q, ExpMapType R[4][4])
{
    ExpMapType  xy, xz, yz;
    ExpMapType  wx, wy, wz;
    ExpMapType  xx, yy, zz;
 ...
 * -----------------------------------------------------------------*/
int Check_Parameterization(ExpMapType v[3], ExpMapType *theta)
{
    int     rep = 0;
    *theta = V3Magnitude(v);
 ...
 * -----------------------------------------------------------------*/
void EM2_To_EM3(ExpMapType r[2], ExpMapType s[3], ExpMapType t[3], ExpMapType v[3])
{
    v[X] = r[X]*s[X] + r[Y]*t[X];
    v[Y] = r[X]*s[Y] + r[Y]*t[Y];
    v[Z] = r[X]*s[Z] + r[Y]*t[Z];
 ...
 * i'th element of 'v'.
 * -----------------------------------------------------------------*/
void Partial_R_Partial_Vi(Quat q, Quat dqdvi, ExpMapType dRdvi[4][4])
{
    ExpMapType    prod[9];
    int       i;
...
void Partial_Q_Partial_3V(ExpMapType v[3], int i, Quat dqdx)
{
	...
	ExpMapType Tsinc = 0.5 - theta*theta/48.0;
	ExpMapType vTerm = v[i] * (theta*theta/40.0 - 1.0) / 24.0;
	...
	const int i2 = (i+1)%3, i3 = (i+2)%3;
	const ExpMapType  ang = 1.0/theta, ang2 = ang*ang*v[i], sang = sinp*ang;
	const ExpMapType  cterm = ang2*(.5*cosp - sang);
 ...
 * -----------------------------------------------------------------*/
void Partial_Q_Partial_2V(ExpMapType r[2], ExpMapType s[3], ExpMapType t[3], int i, Quat dqdx)
{
    int      j;
    Quat     dQdV[3];
    ExpMapType   v[3], *dvdr = i ? t : s;
    ...
    for (j=0; j<4; j++)	
      dqdx[j] = ((ExpMapType*)&dQdV[0])[i]*dvdr[X] + 
	        ((ExpMapType*)&dQdV[1])[i]*dvdr[Y] +
		((ExpMapType*)&dQdV[2])[i]*dvdr[Z];
}
 ...
 * -----------------------------------------------------------------*/
int Partial_R_Partial_EM2(ExpMapType r[3], ExpMapType s[3], ExpMapType t[3],
